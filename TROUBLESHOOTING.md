...
How to understand the codebase, debug issues, and resolve common problems when working with the almanac.
...
**How to query it**:
```bash
# Find all Tier A tools in a category
...
3. Per-category sections: findings, roster, analysis
4. Quest diary: what was tested this month
5. Cross-category findings: patterns that span categories
...
**Symptoms**:
- A tool is Tier A but has no production usage
- A tool is Tier C but is widely adopted
...
**Resolution**:
1. File an issue with evidence (GitHub stars, last push, production references)
...
**Symptoms**:
- A well-known tool is not in the roster
- A tool from a specific region is missing
- A newly launched tool is not in the latest edition
...
3. Check if it falls outside the search scope
...
- `query()` returns nothing or garbage
...
**Symptoms**:
- The no-tool baseline scores above zero
...
A: Yes. The harness is published separately. Clone it, install dependencies, and run the adapter for the tool you want to test. See `TESTING.md` for reproducibility instructions.
...
A: Each category is deep enough to warrant its own dedicated repo with per-tool pages, category-specific benchmarks, and focused community. The parent almanac is the master catalog.

### Q: How often are benchmarks re-run?

A: Standard: every quarter for each category. Stress: annually. Integration: quarterly. If a tool releases a major version, we may re-run early.

### Q: What's the difference between Tier A, B, and C?

A: Tier A = market leader or strongest technical merit. Tier B = solid option, specific use cases. Tier C = niche, early-stage, or specialized. See `IMPLEMENTATION.md` for full rules.
...
- **Tool request**: "Add [Tool Name] to [Category]"
